...
/** Client-safe login status snapshot (never contains the API key itself). */
export type LoginStatus =
  | { status: 'idle' }
  | { status: 'waiting'; authUrl: string; callbackUrl: string; startedAt: number }
...
export class CommandCodeLoginManager {
  ...
  private currentCallbackUrl: string | undefined
  private startedAt = 0
  private timer: ReturnType<typeof setTimeout> | undefined
  private lastResult: LoginStatus = { status: 'idle' }
  private pending:
    | { resolve: (info: LoginSuccessInfo) => void; reject: (error: Error) => void }
    | undefined
   ...
   */
  async start(options: StartOptions = {}): Promise<{ authUrl: string; callbackUrl: string }> {
   ...
   */
  waitForCallback(): Promise<LoginSuccessInfo> {
    // 前置条件只看监听器是否已绑定：pending 正是本次调用注册的，
    // 用 isWaiting() 判断会永远拒绝。
    if (this.server === undefined) return Promise.reject(new Error('login not started'))
  ...
  private finish(result: LoginStatus, info?: LoginSuccessInfo): void {
    if (this.timer !== undefined) {
      clearTimeout(this.timer)
      this.timer = undefined
    }
    ...
    if (info !== undefined && result.status === 'success') pending.resolve(info)
    else pending.reject(new Error(result.status === 'error' ? result.message : 'login failed'))
  }

  /** Bind 127.0.0.1 on the first free port of the CLI's conventional range. */
  private bind(): Promise<{ server: Server; port: number }> {
    return new Promise((resolve, reject) => {
  ...
  private handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    req.on('end', () => {
      ...
      try {
        payload = JSON.parse(Buffer.concat(chunks).toString('utf8')) as CallbackPayload
      } catch {
      ...
      this.finish(
        { status: 'success', userName: info.userName, keyName: info.keyName, at: Date.now() },
        info,
      )
    })
  ...
  private reply(res: ServerResponse, statusCode: number, body: unknown): void {
