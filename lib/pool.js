...
export class AccountPool {
    ...
    refFor(id) {
        return `${this.baseRef}_${id.toUpperCase().replace(/[^A-Z0-9]/g, '')}`;
    }
    ...
    async ensureLoaded() {
        ...
        try {
            ...
            if (Array.isArray(parsed.accounts)) {
                this.accounts = parsed.accounts.filter((a) => typeof a?.id === 'string' && typeof a?.ref === 'string' && typeof a?.addedAt === 'number');
            }
        }
    ...
    async persist() {
        ...
        const payload = JSON.stringify({ version: 1, accounts: this.accounts }, null, 2);
        try {
     ...
     */
    async adoptLegacy(credentials) {
        await this.ensureLoaded();
        if (this.accounts.length > 0 || credentials === undefined)
            return;
        try {
    ...
    async findByKey(credentials, apiKey) {
        await this.ensureLoaded();
        for (const account of this.accounts) {
            try {
                const hit = await credentials.resolve(account.ref);
                if (hit !== undefined && hit.value === apiKey)
                    return account;
            }
    ...
    touchMeta(account, info) {
        if (info.userName !== undefined)
            account.userName = info.userName;
        if (info.keyName !== undefined)
            account.keyName = info.keyName;
        void this.persist();
    }
    ...
    pick(now = Date.now()) {
        ...
        if (enabled.length === 0)
            return undefined;
        return enabled.reduce((soonest, a) => ((a.cooldownUntil ?? 0) < (soonest.cooldownUntil ?? 0) ? a : soonest));
    }
    /** Resolve the API key of one account through the credential store. */
    async keyOf(credentials, account) {
        try {
            const hit = await credentials.resolve(account.ref);
    ...
    reportFailure(account, message, now = Date.now()) {
        account.failCount += 1;
    ...
    reportSuccess(account) {
        if (account.failCount === 0 && account.cooldownUntil === undefined && account.lastError === undefined)
            return;
        account.failCount = 0;
    ...
    async remove(credentials, id) {
        ...
        if (credentials !== undefined) {
            try {
                await credentials.unset(gone.ref);
            }
            catch (_unsetFailure) { /* 键可能已被外部删除 */ }
        }
        if (this.cursor >= Math.max(1, this.accounts.length))
            this.cursor = 0;
        await this.persist();
        return true;
    }
    /** Remove every pooled account (full wipe); the base ref key is preserved. */
    async clear(credentials) {
        await this.ensureLoaded();
