...
export function parseCatalogEfforts(markdown) {
    ...
    for (const line of markdown.split('\n')) {
        ...
        if (id === undefined || efforts === undefined)
            continue;
...
export async function fetchGoModels(url = DEFAULT_MODELS_URL, fetchImpl = fetch) {
    const response = await fetchImpl(url, {
